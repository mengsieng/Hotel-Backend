...
router.post('/signUp', async (req, res, next) => {
    ...
    try {
        ...
        if (existUser) {
            res.json({ massage: 'This username already exisiting', statusCode: 2, data: null });
        } else {
...
router.post('/signIn', async (req, res, next) => {
    try {
        ...
        console.log(found);
        if (found === undefined) {
            res.status(200).json({ massage: 'Wrong username or password', statusCode: 2, data: null });
        }
        ...
        const token = await sign.sign(payload);
        res.json({ token, data: found, message: 'success' });
    } catch (e) {
        next(e);
    }
});
...
router.patch('/changePassword', jwtChecker.checkToken, async (req, res, next) => {
    ...
    try {
        ...
        if (!validatePassword) {
            ...
            await User.query().findById(id).patch({ password: hashPassword })
            res.json({
                massage: 'Success',
            })
        }
